...
def get_frames(clip_info, num_frames, clip_idx):
    ...
    '''裁剪图像'''
    bbox = clip_info['box']
    # bbox_prop = clip_info['box_object']
    if isinstance(bbox[0], list):  # 逐帧框
        ...
        list_frames_clip = [f[:, y1:y2, x1:x2] for f in list_frames]  # 裁剪
    # x11, y11, x22, y22 = bbox_prop
    # list_frames_prop= [f[:, y11:y22, x11:x22] for f in list_frames]
    frames = torch.stack(list_frames_clip)  # 拼接,[T,C,H,W]
    frames = frames.permute(0, 2, 3, 1)  # [T,C,H,W]→[T,H,W,C]
    # frames_prop = torch.stack(list_frames_prop)  # 拼接,[T,C,H,W]
    # frames_prop = frames_prop.permute(0, 2, 3, 1)  # [T,C,H,W]→[T,H,W,C]
    # return frames, frames_prop
    return frames
...
class Virat(Dataset):
    ...
    def _construct_loader(self):
        ...
        # path_to_file = join(self.cfg.DATA.PATH_TO_DATA_DIR, '{}_clip_duration32_stride16_personperson.json'.format(name))
        if name == 'train':
    ...
    def __getitem__(self, index):
        ...
        if self.aug:
            ...
            # frames_object = tensor_normalize(frames_object, self.cfg.DATA.MEAN, self.cfg.DATA.STD)
            # frames_object = frames_object.permute(3, 0, 1, 2)  # [T,H,W,C]→[C,T,H,W]
        label = self._labels[index]
        # if self.mode == 'train':
        ...
        # frames_object = pack_pathway_output(self.cfg, frames_object)
        ...
        '''
        if frames_object[0].shape[-2] == 0 or frames_object[0].shape[-1] == 0:
