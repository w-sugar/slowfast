...
class X3DHead(nn.Module):
    ...
    def _construct_head(self, dim_in, dim_inner, dim_out, norm_module):
        ...
        if self.bn_lin5_on:
            self.lin_5_bn1 = norm_module(
                num_features=dim_out, eps=self.eps, momentum=self.bn_mmt
            )
        self.lin_5_relu1 = nn.ReLU(self.inplace_relu)
        '''
        ...
        # self.reg = nn.Linear(dim_out, 4, bias=True)
    ...
    def forward(self, inputs):
        ...
        x_reg = self.lin_51(x)
        if self.bn_lin5_on:
            x_reg = self.lin_5_bn1(x_reg)
        x_reg = self.lin_5_relu1(x_reg)
        # (N, C, T, H, W) -> (N, T, H, W, C).
        ...
        x = x.view(x.shape[0], -1)
        # x_reg = x_reg.view(x_reg.shape[0], -1)
        return x, {}

class X3DBBNHead(nn.Module):
    """
    X3D head.
    This layer performs a fully-connected projection during training, when the
    input size is 1x1x1. It performs a convolutional projection during testing
    ...
    def __init__(
        ...
        Args:
            ...
            bn_lin5_on (bool): if True, perform normalization on the features
                before the classifier.
        """
        super(X3DBBNHead, self).__init__()
        self.pool_size = pool_size
        self.dropout_rate = dropout_rate
        self.num_classes = num_classes
        self.act_func = act_func
    ...
    def _construct_head(self, dim_in, dim_inner, dim_out, norm_module):
        ...
        self.conv_5_bn1 = norm_module(
            num_features=dim_inner, eps=self.eps, momentum=self.bn_mmt
        )
        self.conv_5_relu1 = nn.ReLU(self.inplace_relu)

        if self.pool_size is None:
            self.avg_pool1 = nn.AdaptiveAvgPool3d((1, 1, 1))
        else:
            self.avg_pool1 = nn.AvgPool3d(self.pool_size, stride=1)
        self.lin_51 = nn.Conv3d(
            dim_inner,
            dim_out,
            kernel_size=(1, 1, 1),
        ...
        # Perform FC in a fully convolutional manner. The FC layer will be
        # initialized with a different std comparing to convolutional layers.
        self.projection = nn.Linear(dim_out * 2, self.num_classes, bias=True)
        ...
        # self.reg = nn.Linear(dim_out, 4, bias=True)
    def forward_train(self, inputs, inputs_sample, l):
        x = self.conv_5(inputs[0])
        x = self.conv_5_bn(x)
        x = self.conv_5_relu(x)
        x = self.avg_pool(x)
        ...
        y = self.conv_51(inputs_sample[0])
        y = self.conv_5_bn1(y)
        y = self.conv_5_relu1(y)
        y = self.avg_pool1(y)
        y = self.lin_51(y)
        if self.bn_lin5_on:
            y = self.lin_5_bn1(y)
        y = self.lin_5_relu1(y)
        # (N, C, T, H, W) -> (N, T, H, W, C).
    ...
    def forward(self, inputs, inputs_sample=None, l=None):
        # In its current design the X3D head is only useable for a single
        # pathway input.
        assert len(inputs) == 1, "Input tensor does not contain 1 pathway"
        if self.training:
            return self.forward_train(inputs, inputs_sample, l)
        else:
